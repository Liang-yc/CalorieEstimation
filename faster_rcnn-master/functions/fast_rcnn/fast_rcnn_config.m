...
%
    ...
    ip.addParamValue('scales',          1000,            @ismatrix);
    % Max pixel size of a scaled input image
    ip.addParamValue('max_size',        1200,           @isscalar);
    % Images per batch
    ip.addParamValue('ims_per_batch',   2,              @isscalar);
    % Minibatch size
    ip.addParamValue('batch_size',      128,            @isscalar);%128=>2
    ...
    %% testing
    ip.addParamValue('test_scales',     1000,            @isscalar);
    ip.addParamValue('test_max_size',   1200,           @isscalar);

...
opts.do_val                 = true; %这个就是设置使用验证集，验证集一般是用来调超参数的。
% model
...
fprintf('set cache folder for each stage\n');
model                       = Faster_RCNN_Train.set_cache_folder(cache_base_proposal, cache_base_fast_rcnn, model);
% generate anchors and pre-calculate output size of rpn network 
fprintf('generate anchors and pre-calculate output size of rpn network\n');
...
fprintf('\n***************\nstage one proposal \n***************\n');
% train
model.stage1_rpn            = Faster_RCNN_Train.do_proposal_train(conf_proposal, dataset, model.stage1_rpn, opts.do_val);%默认迭代40000次
% test
